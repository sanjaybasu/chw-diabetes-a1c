...
# --- A1c treatment effects are read from the analysis output (output/results.json) ---
# This module never hard-codes the estimate; run analysis.py first.
def load_effects():
    res = json.load(open(OUTPUT_DIR / "results.json"))
    p, l = res["primary"], res["primary_concordance_last"]
    return {
        "overall": {"ate": p["ate"], "ci_lo": p["ci_lo"], "ci_hi": p["ci_hi"], "label": "Primary (mean HbA1c)"},
        "overall_last_value": {"ate": l["ate"], "ci_lo": l["ci_lo"], "ci_hi": l["ci_hi"], "label": "Sensitivity (last value)"},
    }
...
if __name__ == "__main__":
    ...
    out = OUTPUT_DIR / "cost_effectiveness.json"
