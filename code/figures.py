...
box(2.5,12.8,5,1.9,"Adults (>=18y) with diabetes\n(ICD-10 E08-E13 or HbA1c >=6.5%)\nidentified for care-team outreach\nVirginia Medicaid MCO, 2022-2025")
box(2.5,10.6,5,1.6,"Identified-for-outreach diabetes cohort\nwith >=1 baseline HbA1c\n(eligible for analysis)")
box(2.5,7.4,5,1.6,"With >=1 baseline and >=1 follow-up HbA1c\n(>=90 d after index): N = 372")
box(2.5,4.2,5,1.6,"Propensity-score-trimmed cohort\nN = 369 (116 treated, 253 control)",fc="#dde9f5")
box(2.0,1.6,6,1.8,"Primary analytic cohort\nMean HbA1c, follow-up 90-365 d\nN = 369 (116 treated, 253 control)",fc="#cfe0f1")
# single vertical connector per gap (one arrowhead each)
arrow(5,12.8,5,12.2); arrow(5,10.6,5,9.0); arrow(5,7.4,5,5.8); arrow(5,4.2,5,3.4)
# right-hand annotations: Treated/Control is a label (no arrow); the two exclusion boxes branch off the connector at their own vertical center
side(8.2,10.7,2.1,1.4,"Treated = care team\nactivated\nControl = targeted,\nnot activated")
side(8.2,8.75,2.1,1.5,"Excluded: no follow-up\nHbA1c 90-365 d after\nindex (addressed by\nIPCW sensitivity)"); arrow(5,9.5,8.2,9.5)
side(8.2,5.95,2.1,1.3,"Excluded: missing\ncovariate (N=1);\nPS outside [0.05,0.95]\n(N=2)"); arrow(5,6.6,8.2,6.6)
